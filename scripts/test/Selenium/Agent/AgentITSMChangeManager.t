...
$Selenium->RunTest(
    sub {
        ...
        # create and test change for each state in AgentITSMChangeManager screen
        for my $ChangeState ( 'requested', 'pending approval', 'approved', 'in progress', 'pending pir' ) {
            ...
            # navigate to AgentITSMChangeManager screen
            $Selenium->VerifiedGet(
                "${ScriptAlias}index.pl?Action=AgentITSMChangeManager;SortBy=ChangeNumber;OrderBy=Down"
            );

            # replace 'space' with '%20' in string, so we can use it as a filter param
